 ...
 */
/**
 * An engine on its way: the road tiles from the station to the fire's street,
 * and how far along them it has driven. The renderer reads this verbatim, so
 * what the player watches is the sim's own progress, not an animation
...
export function truckArrived(fire: Fire): boolean {
  return fire.truck !== null && fire.truck.progress >= fire.truck.path.length - 1;
}
...
function ignite(
  ...
  state.fires.set(id, {
    ...
    covered,
    lastSpread: 0,
    truck: covered ? dispatchTruck(state, building) : null,
  });
...
/**
 * Sends an engine from the nearest fire station to the burning building's
 * street. The route is a plain breadth-first walk over road tiles — fires are
 * rare, so the search costs nothing, and the shortest path is the honest one.
 * Null when no road leads there: coverage without a connection is a promise
 * the brigade cannot keep on camera.
 */
function dispatchTruck(state: GameState, building: Building): TruckRun | null {
  let station: { x: number; y: number } | null = null;
  let best = Infinity;
  for (const service of state.services.values()) {
    if (service.kind !== 'fire') continue;
    const d = Math.abs(service.x - building.x) + Math.abs(service.y - building.y);
    if (d < best) {
      best = d;
      station = service;
    }
  ...
  const to = nearestRoadTile(state.world, building.x, building.y);
...
function nearestRoadTile(world: World, x: number, y: number): { x: number; y: number } | null {
  for (let r = 0; r <= 4; r++) {
...
const FIRE_STEPS: readonly [number, number][] = [
  [1, 0],
  [-1, 0],
  [0, 1],
  [0, -1],
