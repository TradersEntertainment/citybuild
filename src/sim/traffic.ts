...
export interface TrafficField {
  ...
  /**
   * Riders a minute the transit network is carrying (sim/transit.ts).
   *
   * Measured here rather than in the economy because it is the same sum: the
   * trips taken off the road are exactly the trips that paid a fare, and
 ...
 */
/** Trips a minute generated by whatever building stands on a tile. */
function tripsAtTile(state: GameState, x: number, y: number): number {
  const id = state.world.building[index(state.world, x, y)] ?? 0;
  const building = id === 0 ? undefined : state.buildings.get(id);
  if (!building) return 0;
  return building.zone === 'res'
...
export function computeTraffic(state: GameState, fields: Fields, traffic: TrafficField): void {
  ...
  // less whatever share of them goes by bus.
